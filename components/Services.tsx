        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {isLoading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="clay-card p-10 flex flex-col items-start space-y-4">
                <div className="w-16 h-16 clay-card-inset skeleton rounded-2xl"></div>
                <div className="h-6 w-3/4 skeleton rounded-lg"></div>
                <div className="h-20 w-full skeleton rounded-xl"></div>
                <div className="h-10 w-full skeleton rounded-2xl"></div>
              </div>
            ))
          ) : (
            items.map((service) => {
