        ...
        <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16">
          ...
          <div className="flex flex-wrap gap-4">
            {['All', 'Graphic Design', 'Web Development'].map((cat) => (
              <button key={cat} onClick={() => setFilter(cat as any)} className={`px-6 py-2 rounded-full font-bold transition-all ${filter === cat ? 'clay-button-primary' : 'clay-button'}`}>
                {cat}
              </button>
            ))}
          </div>
            ...
            <div key={project.id} className="clay-card overflow-hidden group">
              <div className="relative aspect-[4/3] overflow-hidden m-4 rounded-[1.5rem] shadow-inner">
                ...
                <div className="absolute inset-0 bg-indigo-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <button className="clay-button-primary px-6 py-2 font-bold transform translate-y-4 group-hover:translate-y-0 transition-transform">View Project</button>
                </div>
              </div>
